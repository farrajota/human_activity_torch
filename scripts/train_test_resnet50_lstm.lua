--[[
    Train and test the resnet 50 + LSTM network.
]]


local load_logger, get_configs = paths.dofile('opts.lua')

local logger_filename = paths.concat(projectDir,'exp', 'ucf_sports', 'resnet50_hms_lstm.log')
local logger = load_logger(logger_filename)

local opts = {
    {expID = 'resnet50-lstm', netType = 'resnet50-lstm'},
}
