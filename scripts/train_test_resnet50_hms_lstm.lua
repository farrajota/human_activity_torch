--[[
    Train and test the resnet50 + Pose + LSTM network.
]]


local load_logger, get_configs = paths.dofile('opts.lua')

local logger_filename = paths.concat(projectDir,'exp', 'ucf_sports', 'resnet50_lstm.log')
local logger = load_logger(logger_filename)

local opts = {
    {expID = 'resnet50-hms-lstm', netType = 'resnet50-hms-lstm'},
}
