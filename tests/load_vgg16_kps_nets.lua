--[[
    Test loading the vgg16 + kps models.
]]
...
local nets = {'vgg16-kps-lstm', 'vgg16-kps-convnet3d'}  -- vgg16-kps-convnet3d
for k, netType in ipairs(nets) do
    ...
    -- convert table into a single tensor
    local inputs_kps = nn.JoinTable(1):cuda():forward(batch_feats_imgs)


    -- classify the sequence of images
    local res = model_classifier:forward({inputs_feats, inputs_kps})
