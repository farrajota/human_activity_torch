...
local function Parse(arg)
    ...
    cmd:text()
    cmd:option('-expID',    'resnet50-hms-lstm', 'Experiment ID')
    cmd:option('-dataset',  'ucf_sports', 'Dataset choice: ucf_sports.')
    ...
    cmd:text()
    cmd:option('-netType',  'resnet50-hms-lstm', 'Options: vgg16-lstm | hms-lstm | vgg16-hms-lstm | vgg16-convnet | hms-convnet | vgg16-hms-convnet.')
    cmd:option('-nFeats',            512, 'Number of features of the rnn/conv layer.')
    ...
    cmd:option('-nEpochs',           30, 'Total number of epochs to run')
    cmd:option('-seq_length',        30, 'Sequence length (number of frames per window)')
    ...
    cmd:option('-scale',              .10, 'Degree of scale augmentation')
    ...
    cmd:option('-heatmap_jit',          6, 'Jitters the heatmaps (x,y) coordinates around by N pixels.')
    cmd:option('-same_transform_heatmaps', 'true', 'Use the same transformations for all images of a sequence (heatmap generation).')
    cmd:option('-same_transform_features', 'false', 'Use the same transformations for all images of a sequence (feature generation).')
    cmd:option('-use_center_crop', 'true', 'Use center crops for feature extraction.')
    cmd:text()
    cmd:text(' ---------- Test options ---------------------------------------')
    cmd:text()
    cmd:option('-test_seq_length',         90, 'Test sequence length (number of frames per window)')
